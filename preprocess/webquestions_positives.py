...
def load_webq_tsv_pairs(tsv_path):
    ...
    with open(tsv_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            question, answer = row['sentence1'], row['sentence2']
...
def process_webq_tsv(split_name, tsv_path, model):
    ...
    for (q, a), q_emb, a_emb in tqdm(zip(pairs, question_embs, answer_embs), total=len(pairs)):
        sim = cosine_similarity(
            np.array(q_emb).reshape(1, -1),
            np.array(a_emb).reshape(1, -1)
        )[0][0]
        similarity_dict[(q, a)] = [float(sim)]

    output_file = os.path.join(OUTPUT_DIR, f"webq_{split_name}_pos_emb.pkl")
...
def main():
    webq_paths = {
        ...
        "test_path": "../dataset/web_questions/val/positives.tsv",
    }
