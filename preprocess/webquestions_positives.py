...
def webquestion_template_context(query, passage):
    return (
...
def create_finetuning_data_optimized(positive_tsv_path, num_positive_samples=5000, num_negative_samples=5000):
    ...
    with open(positive_tsv_path, 'r', encoding='utf-8') as f:
        ...
        for i, row in enumerate(reader):
            ...
            if question and answer:
                template = webquestion_template_context(question, answer)
    ...
    while len(negative_pairs) < num_negative_samples and attempts < max_attempts:
        ...
        template = webquestion_template_context(q, a)
...
def fine_tune_distilbert_classifier(train_texts, train_labels, val_texts=None, val_labels=None,
                                    output_dir='./distilbert-webq-classifier'):
...
def generate_positive_logits_and_features_distilbert(tsv_path, output_path, finetuned_model_path, batch_size=32,
    ...
    all_templates = [webquestion_template_context(q, a) for q, a in pairs]
...
def create_simple_positive_logits_final():
    ...
    for split in ['train', 'val']:
        input_path = f'../outputs/pos_emb/webq_{split}_pos_emb.pkl'
        output_path = f'../outputs/pos_emb/webq_{split}_pos_logits.pkl'
...
def main():
    ...
    train_texts, train_labels = create_finetuning_data_optimized(
        "../dataset/web_questions/train/positives.tsv",
    ...
    val_texts, val_labels = create_finetuning_data_optimized(
        "../dataset/web_questions/val/positives.tsv",
    ...
    train_output = generate_positive_logits_and_features_distilbert(
        tsv_path="../dataset/web_questions/train/positives.tsv",
        # 🔥 UPDATED: Use the same output paths as your original code
        output_path="../outputs/pos_emb/webq_train_pos_emb.pkl",
        finetuned_model_path=finetuned_model_path
    )

    print("\n=== VALIDATION GENERATION ===")
    val_output = generate_positive_logits_and_features_distilbert(
        tsv_path="../dataset/web_questions/val/positives.tsv",
        # 🔥 UPDATED: Use the same output paths as your original code
        output_path="../outputs/pos_emb/webq_val_pos_emb.pkl",
        finetuned_model_path=finetuned_model_path
    )
    ...
    print(f"   - ../outputs/pos_emb/webq_train_pos_emb.pkl (Full data with features)")
    print(f"   - ../outputs/pos_emb/webq_val_pos_emb.pkl (Full data with features)")
    print(f"   - ../outputs/pos_emb/webq_train_pos_logits.pkl (Training logits - FLAT DICT FORMAT)")
    print(f"   - ../outputs/pos_emb/webq_val_pos_logits.pkl (Training logits - FLAT DICT FORMAT)")
