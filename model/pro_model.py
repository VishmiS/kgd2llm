...
class Mymodel(nn.Module):
    ...
    def forward(self, inputs_all, task_ids, mode):
        if mode == 'train':
            output_embeddings_all = self.get_sentence_embedding(**inputs_all).reshape(2 + self.args.neg_K, -1,
                                                                                      self.emb_dim)
    ...
    def encode(self, sentences, batch_size=64, convert_to_numpy=True,
        ...
        with torch.no_grad():
            for start_index in trange(0, len(sentences), batch_size, desc="Batches", disable=not show_progress_bar):
                ...
                with torch.no_grad():
                    inputs = self.tokenizer(sentences_batch, padding=True, truncation=True, max_length=max_seq_length,
                                            return_tensors='pt').to(self.plm_model.device)
