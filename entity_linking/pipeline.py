...
def falcon_entity_linking(text):
    """
    Use Falcon 2.0 API to detect and link entities from the input text.
    ...
    if response.status_code == 200:
        ...
        wikidata_entities = {
            e['URI'].split('/')[-1]: e['surface form']
            for e in entities_wd if 'URI' in e
        }
        dbpedia_entities = {
            e['URI']: e['surface form']
            for e in entities_db if 'URI' in e
        }

        return wikidata_entities, dbpedia_entities
    else:
...
def enrich_query_with_aliases_and_facts(original_text):
    # ------------------- Entity Linking -------------------
    falcon_qids, falcon_dbpedia = falcon_entity_linking(original_text)
    dbpedia_entities = falcon_dbpedia or dbpedia_entity_linking(original_text)
