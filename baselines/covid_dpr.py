...
# python /root/pycharm_semanticsearch/baselines/covid_dpr.py
...
class DPREvaluator:
    ...
    def evaluate(self, k=10):
        """
        Evaluate DPR on MRR@k and Recall@k with graded relevance
        ...
        for query_id, query_text in self.queries.items():
            ...
            # Consider documents with relevance > 0 as relevant for binary metrics
            binary_relevant_docs = {doc_id for doc_id, rel in relevant_docs.items() if rel > 0}

            if not binary_relevant_docs:
            ...
            # Calculate MRR@k (binary relevance)
            mrr = self._calculate_mrr(top_k_doc_ids, binary_relevant_docs)
            mrr_scores.append(mrr)

            # Calculate Recall@k (binary relevance)
            recall = self._calculate_recall(top_k_doc_ids, binary_relevant_docs)
