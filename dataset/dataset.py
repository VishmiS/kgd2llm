...
DATASET_ID_DICT = {'snli': 1, 'multinli': 2, 'msmarco': 3}
...
def collate_fn(data):
    ...
    return res_s_a, res_s_b, torch.FloatTensor(res_pos_logits), res_neg_K, torch.FloatTensor(
        res_neg_logits), torch.LongTensor(res_task_id)
        ...
        for name in names:
            if name in ['snli']:
                if name == 'snli':
                    ...
                    self.data, sample_num = load_text_dataset(name, os.path.join(pos_dir, 'PATH_TO_DATA'),
                                                              os.path.join(neg_dir, 'PATH_TO_DATA'),
                                                              os.path.join(datadir, 'PATH_TO_DATA'), self.neg_K,
                                                              self.data, 'train')
                ...
                if name == 'sts':
                    start_id = len(self.data)
                    self.data, sample_num = load_sts_dataset_train(name, os.path.join(pos_dir, 'PATH_TO_DATA'),
                                                                   os.path.join(neg_dir, 'PATH_TO_DATA'), datadir,
                ...
                if name == 'mmarco':
                    start_id = len(self.data)
                    self.data, sample_num = load_mmarco_dataset_train(name, os.path.join(pos_dir, 'PATH_TO_DATA'),
                                                                      os.path.join(neg_dir, 'PATH_TO_DATA'),
                                                                      os.path.join(datadir, 'PATH_TO_DATA'), self.neg_K,
                                                                      self.data)
                    end_id = len(self.data)
    ...
    def __init__(self, tokenizer, pos_dir, neg_dir, datadir, names=None, batch_size=32, neg_K=8, process_index=0,
        ...
        for name in names:
            if name in ['snli-zh']:
                ...
                if name == 'sts':
                    start_id = len(self.data)
                    self.data, sample_num = load_sts_dataset_val(name, os.path.join(pos_dir, 'PATH_TO_DATA'),
                                                                 os.path.join(neg_dir, 'PATH_TO_DATA'),
                                                                 os.path.join(datadir, 'PATH_TO_DATA'), self.neg_K,
                                                                 self.data)
                    end_id = len(self.data)
                ...
                if name == 'mmarco':
                    start_id = len(self.data)
                    self.data, sample_num = load_mmarco_dataset_val(name, os.path.join(pos_dir, 'PATH_TO_DATA'),
                                                                    os.path.join(neg_dir, 'PATH_TO_DATA'),
                                                                    os.path.join(datadir, 'PATH_TO_DATA'), self.neg_K,
                                                                    self.data)
                    end_id = len(self.data)
